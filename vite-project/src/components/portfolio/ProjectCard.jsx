...
export default function ProjectCard({ project }) {
  ...
  // The Bypass Switch: Wraps the card in a hyperlink ONLY if a pageLink is provided
  const CardWrapper = ({ children }) => {
    if (project.pageLink) {
      return (
        <Link to={project.pageLink} className="block cursor-pointer">
          {children}
        </Link>
      );
    }
  ...
  // Layout for Text OR Image (No Videos)
  if (!project.videos) {
    return (
      <CardWrapper>
        <div className="group relative rounded-xl border border-border bg-card overflow-hidden hover:border-primary/30 transition-all duration-300">
          ...
          {/* Renders the image at the top if one exists */}
          {project.image && (
            <img 
              src={project.image} 
              alt={project.title} 
